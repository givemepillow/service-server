...
async def registration(request):
    if await Verify.is_verified(request.data.email, request.data.login):
        try:
            ...
            return AnswerConstructor.create(ResponseType.ERROR, message='Ошибка расшифровки пароля.')
        ...
        if db_answer:
            ...
            return AnswerConstructor.create(ResponseType.ACCEPT, message='Регистрация завершена.')
        else:
            logger.info(
                f'Отклонена регистрация нового пользователя (Недопустимые данные.) {request.data.email}: {request.ip}')
            return AnswerConstructor.create(ResponseType.REJECT, message='Недопустимые данные.')
    else:
        logger.warning(
        ...
        return AnswerConstructor.create(ResponseType.REJECT, message='Почта или логин не подтверждены.')
